...
O "Bitcoin Puzzle Transaction" (tx 08389f34...) bloqueou valores em
enderecos cujas chaves privadas estao em intervalos determinados:

    puzzle n  ->  intervalo [ 2^(n-1),  2^n - 1 ]
                  (bits de 1 ate n, com o bit mais significativo ligado)

A tabela de enderecos abaixo e publica e amplamente replicada. Foi
validada contra a API blockstream.info (o puzzle 66, por exemplo,
confirma saldo de ~6.6 BTC). Fonte de referencia:
    https://github.com/Daniel-Wu-1/bitcoin-puzzle-gpu

Nenhum endereco aqui e "descoberta" deste projeto - sao dados publicos
do desafio original.
"""
...
def puzzle_range(number):
    """Intervalo de chaves do puzzle n: [2^(n-1), 2^n - 1]."""
    if number < 1 or number > MAX_PUZZLE:
        raise ValueError(f"puzzle deve estar entre 1 e {MAX_PUZZLE}")
    return (1 << (number - 1), (1 << number) - 1)


def puzzle_address(number):
    """Endereco alvo publico do puzzle n."""
    try:
        return PUZZLE_ADDRESSES[number]
    except KeyError as exc:
        raise ValueError(f"puzzle deve estar entre 1 e {MAX_PUZZLE}") from exc


def all_puzzles():
    """Itera por (n, inicio, fim, endereco) para todos os 160 puzzles."""
    for n in range(1, MAX_PUZZLE + 1):
        start, end = puzzle_range(n)
        yield n, start, end, PUZZLE_ADDRESSES[n]


def to_hex(value):
    """Formata inteiro como hex 0x sem zeros a esquerda desnecessarios."""
    return hex(value)


def from_hex(value):
    """Converte '0x...' ou decimal em int."""
    return int(value, 0)
